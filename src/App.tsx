...
export default function App() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => {
    const saved = localStorage.getItem("ruhi_chat_history");
    if (saved) {
      try {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("ruhi_chat_history", JSON.stringify(messages));
  }, [messages]);
  ...
  const handleTextCommand = useCallback(async (finalTranscript: string) => {
    ...
    if (commandResult.isBrowserAction) {
      ...
      setMessages((prev) => [...prev, { id: Date.now().toString() + "-r", sender: "ruhi", text: responseText }]);
      ...
      setTimeout(() => {
        if (commandResult.webhookUrl) {
          // Fire MacroDroid Webhook silently in the background
          fetch(commandResult.webhookUrl, { mode: 'no-cors' }).catch((err) =>
            console.error("Webhook failed to trigger", err)
          );
      ...
      responseText = await getRuhiResponse(finalTranscript, messagesRef.current, accent);
      setMessages((prev) => [...prev, { id: Date.now().toString() + "-r", sender: "ruhi", text: responseText }]);
  ...
  const toggleListening = async () => {
    if (isSessionActive) {
      ...
      try {
        ...
        const session = new LiveSessionManager(accent);
    ...
    <div className="h-[100dvh] w-screen bg-[#050505] text-white flex flex-col items-center justify-between font-sans relative overflow-hidden m-0 p-0">
      ...
      <header className="absolute top-0 left-0 w-full flex justify-between items-center z-20 shrink-0 px-6 py-4 md:px-12 md:py-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-serif font-medium tracking-wide opacity-90">Ruhi</h1>
        </div>
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="bg-white/5 border border-white/10 rounded-full text-sm px-3 py-1.5 focus:outline-none focus:border-violet-500/50 transition-colors opacity-90 cursor-pointer appearance-none text-white text-center"
            title="Choose Indian Accent"
          >
            <option value="default" className="bg-neutral-900 text-white">Default Indian</option>
            <option value="south" className="bg-neutral-900 text-white">South Indian</option>
          ...
          {showTextInput && (
            ...
            >
              <input 
                ...
                placeholder="Type a message to Ruhi..."
